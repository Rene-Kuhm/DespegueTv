...
fun SourceStatusFilterChip(
    ...
    val textColor = when {
        isError -> DespegueColors.Error
        isFocused || isSelected -> DespegueColors.OnSecondary
    ...
    FilterChip(
        ...
        colors = FilterChipDefaults.colors(
            containerColor = if (shouldUseNormalColors) DespegueColors.BackgroundCard else DespegueColors.Error.copy(alpha = 0.05f),
            focusedContainerColor = if (shouldUseNormalColors) DespegueColors.Secondary else DespegueColors.Error.copy(alpha = 0.08f),
            selectedContainerColor = if (shouldUseNormalColors) DespegueColors.Secondary else DespegueColors.Error.copy(alpha = 0.07f),
            focusedSelectedContainerColor = if (shouldUseNormalColors) DespegueColors.Secondary else DespegueColors.Error.copy(alpha = 0.09f),
        ...
        border = FilterChipDefaults.border(
            border = Border(
                border = BorderStroke(1.dp, if (isError) DespegueColors.Error.copy(alpha = 0.7f) else DespegueColors.Border),
                shape = RoundedCornerShape(20.dp)
            ),
            focusedBorder = Border(
                border = BorderStroke(2.dp, if (isError) DespegueColors.Error.copy(alpha = 0.8f) else DespegueColors.FocusRing),
                shape = RoundedCornerShape(20.dp)
            ),
            selectedBorder = Border(
                border = BorderStroke(1.dp, if (isError) DespegueColors.Error.copy(alpha = 0.75f) else DespegueColors.Primary),
                shape = RoundedCornerShape(20.dp)
            ),
        ...
        Row(
            ...
            if (isLoading) {
                MaterialCircularProgressIndicator(
                    ...
                    color = if (isFocused || isSelected) DespegueColors.OnSecondary else DespegueColors.TextSecondary,
                    strokeWidth = 1.5.dp
                )
            }
            Text(
