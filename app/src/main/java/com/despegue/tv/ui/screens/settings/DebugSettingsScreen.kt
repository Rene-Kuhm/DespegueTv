...
import com.despegue.tv.ui.screens.account.InputField
...
private fun DebugDialogButton(
    ...
    Card(
        ...
        Text(
            text = text,
            style = MaterialTheme.typography.bodyMedium,
            color = if (isFocused) DespegueColors.TextPrimary else DespegueColors.TextSecondary,
            modifier = Modifier
                .fillMaxWidth()
    ...
    Column(
        ...
        if (result != null) {
            Text(
                text = result,
                style = MaterialTheme.typography.bodySmall,
                color = if (result.startsWith("Failed")) DespegueColors.Error else DespegueColors.Secondary
            )
        }
